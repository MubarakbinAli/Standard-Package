    ...
    if (isSupabaseConfigured) {
      ...
      try {
        ...
        if (error) {
          ...
          // Check for "relation does not exist" error (missing table)
          if (error.code === '42P01' || error.message.includes('Could not find the table') || error.message.includes('relation "public.bookings" does not exist')) {
            console.group("⚠️ TABLE MISSING");
            console.warn("The 'bookings' table was not found in your Supabase project.");
            console.info("Please run the following SQL command in your Supabase Dashboard > SQL Editor:");
            console.log(`%c${CREATE_TABLE_SQL}`, 'color: #00e676; font-weight: bold; padding: 5px; background: #222; border-radius: 4px;');
            console.groupEnd();
          } else if (error.code === '42501') {
            console.group("⚠️ PERMISSION DENIED");
            console.warn("Row Level Security (RLS) policy violation.");
            console.info("Please run this SQL to enable public inserts:");
            console.log(`%ccreate policy "Enable insert for all users" on public.bookings for insert with check (true);`, 'color: #00e676; font-weight: bold; padding: 5px; background: #222; border-radius: 4px;');
            console.groupEnd();
          }
        } else {
